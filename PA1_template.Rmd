...
```{r, options(scipen = 1, digits = 0, cache = TRUE)}
data <- read.csv("activity.csv", header = TRUE, na.strings=c("NA"), comment.char = "")
##The number of days in the data before any processing is 61 days (date variable is 61 levels)
...
Considering complete cases only, the mean total number of steps taken per day is `r mean_steps`.
...
hist(sumperday, col = "red", breaks = 12, ylim = c(0, 20), xlim = c(0, 25000), main = "Total number of steps taken each day", xlab = "Number of steps", ylab = "Frequency (number of days)")
rug(sumperday)
```
...
Considering complete cases only, the mean number of steps per day is `r mean(sumperday)`.

```{r}
print(median(sumperday)) ## Median steps per day (complete cases)
```

Considering complete cases only, the median number of steps per day is `r median(sumperday)`.
...
plot(meanperinterval ~ Interval, data = meanperinterval, col = "blue", type = "l", main = "Time series plot", xlab = "Time interval", ylab = "Average steps per interval")
```
...
The 5-minute interval, on average across all the days in the dataset, that contains the maximum number of steps is interval `r maximum_number_of_steps_interval`.
...
The number of missing values is `r n.missing`.
...
hist(imputed_sumperday, col = "red", breaks = 12, ylim = c(0, 20), xlim = c(0, 25000), main = "Total number of steps taken each day", xlab = "Number of steps", ylab = "Frequency (number of days)")
...
Considering imputet dataset, the mean number of steps per day is `r mean(imputed_sumperday)`.

```{r}
print(median(imputed_sumperday)) ## Median steps per day (imputed dataset)
```

Considering imputet dataset, the mean number of steps per day is `r median(imputed_sumperday)`.
...
dayclass <- day
levels(dayclass) <- c("weekday", "weekday", "weekend", "weekend","weekday", "weekday", "weekday") ## Levels of dayclass: weekday weekend
...
```{r}
imputed_data_list <- split(imputed_data$imputed_steps, list(imputed_data$interval, imputed_data$dayclass))  ## Generate an interaction factor between the interval and the dayclass
...
library(lattice)
xyplot(meanperinterval~Interval | dayclass, data = meanperinterval, type = "l", ylab = "Number of steps",layout = c(1,2))
```
## Part-3 (Comparison): 
...
```{r}
par(mfrow = c(1,2))
hist(sumperday, col = "red", breaks = 12, ylim = c(0, 20), xlim = c(0, 25000), main = "Complete cases", xlab = "Number of steps", ylab = "Frequency (number of days)")
hist(imputed_sumperday, col = "red", breaks = 12, ylim = c(0, 20), xlim = c(0, 25000), main = "Imputed missing values", xlab = "Number of steps", ylab = "Frequency (number of days)")
```
